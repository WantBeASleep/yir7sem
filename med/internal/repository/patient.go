...
func (p *PatientRepo) UpdatePatient(ctx context.Context, PatientInfo *entity.PatientInformation) error {
	...
	PatientDB := mapper.PatientToModel(PatientInfo.Patient)
	if err := tx.Model(&models.Patient{}).Where("id = ?", PatientDB.UUID).Updates(&PatientDB).Error; err != nil {
		tx.Rollback()
...
// FIXME:
func (p *PatientRepo) GetPatientInfoByID(ctx context.Context, UUID string) (*entity.PatientInformation, error) {
	var respPatient *models.Patient
	// var respCard *models.PatientCard

	tx := p.db.WithContext(ctx)
	if err := tx.Model(&models.Patient{}).Where("id = ?", UUID).First(&respPatient).Error; err != nil {
		tx.Rollback()
		return nil, fmt.Errorf("failed to update patient info: %w", err)
	}
